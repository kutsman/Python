...
class Printer(OfficeTech):
    def view(self):
        if type(self.pcs) == int:
            return f"{self.brand} {self.price} {self.pcs} {self.serial}"
        else:
            return f"{self.brand} {self.price} (!Количество единиц введите в виде числа, а не строки) {self.serial}"
...
store.add_tech(Scaner('Epson', 9500, 1, 'GW1-200'))
store.add_tech(Xerox('Xerox', 19500, 2, 'X15-9000'))
store.show_store()
store1 = Store('Офис продаж')
store1.add_tech(Printer('Hewlett', 10000, 3, 'HRW-1000'))
store1.add_tech(Printer('Epson', 12360, 7, 'GW1-105'))
store1.add_tech(Scaner('Epson', 9500, 1, 'GW1-200'))
store1.show_store()
store.move_tech('АХО', Xerox('Xerox', 19500, 2, 'X15-9000'))
store.unit_items()
store.move_tech('АХО', Scaner('Epson', 9500, 1, 'GW1-200'))
store.unit_items()
store.show_store()
